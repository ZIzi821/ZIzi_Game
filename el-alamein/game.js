(function () {
  ...
  const aiHeuristicsPromise = import("./src/app/ai-heuristics.js?v=20260707-ai-heuristics-1");
  const HIGHLIGHT = {
  ...
  function scoreAiHex(unit, hexId, route = null) {
    ...
    if (side === "axis") {
      ...
      score += axisForwardZocLineScore(unit, hexId) * (assault ? 1.25 : combat >= 3 ? 1.7 : 0.75);
      ...
      score += alliedForwardZocWallScore(unit, hexId) * (combat >= 3 ? 1.45 : 1.05);
  ...
  function alliedForwardZocWallScore(unit, hexId) {
    ...
    return app.aiHeuristics.clampScore(score, -120, 440);
  }
