...
function playGame(gameSeed) {
  ...
  while (!state.winner && guard < 80) {
    ...
    observeAxisObjectiveRetention(state);
...
function currentOperationalPlan(state, side) {
  if (
    state.movementPlan
    && state.movementPlan.side === side
    && state.movementPlan.turn === state.turn
    && state.movementPlan.phaseIndex === state.phaseIndex
  ) {
    return state.movementPlan;
  }
  state.movementPlan = buildOperationalPlan(state, side);
  return state.movementPlan;
}

function buildOperationalPlan(state, side) {
  const sideUnits = liveUnits(state.units).filter((unit) => unit.side === side && !unit.disrupted);
  if (side === "axis") {
    const focusObjectiveHex = axisFocusObjective(state);
    const westExitThreat = Math.max(0, ...scenario.objectives.alliedWestExitEdge.map((hexId) => alliedExitThreat(state, hexId)));
    const ranked = sideUnits
    ...
    return {
      ...
      focusObjectiveHex,
      spearheadUnitIds: new Set(ranked.slice(0, Math.max(5, Math.min(8, ranked.length))).map((unit) => unit.id)),
      westExitThreat,
    };
  }

  const axisSpearheads = liveUnits(state.units)
    .filter((unit) => isAxisAssaultUnit(unit) && !unit.disrupted)
    .sort((a, b) => nearestDistance(a.hexId, axisObjectives()) - nearestDistance(b.hexId, axisObjectives()));
  const wallUnitIds = new Set(sideUnits
    ...
    .slice(0, Math.max(6, Math.min(10, sideUnits.length)))
    .map((unit) => unit.id));
  return {
    side,
    turn: state.turn,
    phaseIndex: state.phaseIndex,
    focusObjectiveHex: axisFocusObjective(state),
    axisSpearheadIds: new Set(axisSpearheads.slice(0, 6).map((unit) => unit.id)),
    wallUnitIds,
  };
}
...
function chooseRetreat(state, unit, paths, controllerSide) {
  ...
  for (const [hexId, path] of paths.entries()) {
    const retreaterScore = scoreHex(state, unit, hexId, { remaining: 0, path })
      ...
      + ownRetreatHold(state, unit, hexId);
...
function forcedRetreatDenialScore(state, controllerSide, unit, hexId) {
  ...
  if (controllerSide === "allied" && unit.side === "axis") {
    ...
    return objectiveDistance * 6
      + trapScore
      - strategicHexValue(state, "axis", hexId) * 0.12
      + forcedRetreatObjectiveDenialScore({
        controllerSide,
        retreatingSide: unit.side,
        isAxisObjective: axisObjectives().includes(hexId),
...
function ownRetreatHold(state, unit, hexId) {
  if (unit.side !== "axis") return 0;
  return objectiveRetreatHoldScore({
    isObjective: axisObjectives().includes(hexId),
    combat: Number(unit.combat || 0),
    supportStrength: sideStrengthWithin(state, "axis", hexId, 2, unit.id),
    adjacentSupportCount: sideUnitsWithin(state, "axis", hexId, 1, unit.id).length,
    counterattackThreat: counterattackThreatAgainstHex(state, "allied", hexId),
  });
}

function sideUnitsWithin(state, side, hexId, radius, ignoreUnitId = null) {
  return liveUnits(state.units).filter((unit) => (
    unit.id !== ignoreUnitId
    && unit.side === side
    && !unit.disrupted
    && distance(unit.hexId, hexId) <= radius
  ));
}

function sideStrengthWithin(state, side, hexId, radius, ignoreUnitId = null) {
  return sideUnitsWithin(state, side, hexId, radius, ignoreUnitId)
    .reduce((sum, unit) => sum + Number(unit.combat || 0), 0);
}

function counterattackThreatAgainstHex(state, side, hexId) {
  return liveUnits(state.units)
    .filter((unit) => unit.side === side && !unit.disrupted)
    .reduce((sum, unit) => {
      const unitDistance = distance(unit.hexId, hexId);
      const reach = Math.max(movementAllowance(state, unit), Number(unit.movement || 0)) + 1;
      if (unitDistance > reach) return sum;
      return sum + Number(unit.combat || 0) * (unitDistance <= 2 ? 1.2 : 1);
    }, 0);
}
...
function alliedSpearheadCounterattackUrgency(state, attackerSide, attackers, defender, odds) {
  ...
  const nearbyAxisSupport = liveUnits(state.units)
    .filter((unit) => unit.side === "axis" && unit.id !== defender.id && !unit.disrupted && distance(unit.hexId, defender.hexId) <= 2)
    .reduce((sum, unit) => sum + Number(unit.combat || 0), 0);
...
function scoreHex(state, unit, hexId, route = null) {
  ...
  if (unit.side === "axis") {
    ...
    score += axisObjectiveEntrySecurity(state, unit, hexId);
    ...
    score += axisSpearheadPlaybookMove(state, unit, hexId, route) * (assault ? 0.82 : 0.48);
    score += axisOperationalPlanMove(state, unit, hexId) * (assault ? 0.72 : 0.5);
    ...
    score += alliedFinalGateScreen(state, unit, hexId) * (combat >= 3 ? 1.45 : 1.06);
    ...
    score += alliedForwardZocWall(state, unit, hexId) * (combat >= 3 ? 2.7 : 1.95);
    score += alliedOperationalPlanMove(state, unit, hexId) * (combat >= 3 ? 0.36 : 0.24);
...
function axisObjectiveEntrySecurity(state, unit, hexId) {
  if (unit.side !== "axis" || !axisObjectives().includes(hexId)) return 0;
  return objectiveEntrySecurityScore({
    isObjective: true,
    turn: state.turn,
    combat: Number(unit.combat || 0),
    supportStrength: sideStrengthWithin(state, "axis", hexId, 2, unit.id),
...
function axisSpearheadPlaybookMove(state, unit, hexId, route = null) {
  if (unit.side !== "axis") return 0;
  const focus = axisFocusObjective(state);
  if (!focus) return 0;
  const lineLinks = liveUnits(state.units).filter((ally) => (
    ...
    && distance(ally.hexId, focus) <= 7
  )).length;

  return axisSpearheadPlaybookScore({
    turn: state.turn,
    currentFocusDistance: distance(unit.hexId, focus),
    candidateFocusDistance: distance(hexId, focus),
    currentObjectiveDistance: nearestDistance(unit.hexId, axisObjectives()),
    candidateObjectiveDistance: nearestDistance(hexId, axisObjectives()),
    movement: Number(unit.movement || 0),
    combat: Number(unit.combat || 0),
...
function axisOperationalPlanMove(state, unit, hexId) {
  if (unit.side !== "axis") return 0;
  const plan = currentOperationalPlan(state, "axis");
  const focus = plan?.focusObjectiveHex || axisFocusObjective(state);
  if (!focus) return 0;
  ...
  const role = plan.spearheadUnitIds?.has(unit.id)
    ...
    : combat >= 3 || movement >= 6
      ? "support"
      : "screen";
  const mates = liveUnits(state.units).filter((ally) => ally.side === "axis" && ally.id !== unit.id && !ally.disrupted);
  const lineLinks = mates.filter((ally) => distance(hexId, ally.hexId) === 2 && nearestDistance(ally.hexId, axisObjectives()) <= 12).length;
  const adjacentCrowd = mates.filter((ally) => distance(hexId, ally.hexId) === 1 && nearestDistance(ally.hexId, axisObjectives()) <= 12).length;
  const pocket = bestAxisOperationalPocket(state, unit, hexId, focus);

  return axisOperationalPlanMoveScore({
    turn: state.turn,
    role,
    currentFocusDistance: distance(unit.hexId, focus),
    candidateFocusDistance: distance(hexId, focus),
    ...
    localOvermass: axisLocalAttackOvermass(state, unit, unit.hexId),
    westExitDistance: nearestDistance(hexId, scenario.objectives.alliedWestExitEdge),
    westExitThreat: plan?.westExitThreat || 0,
    inEnemyZoc: isEnemyZoc(context(state), hexId, unit.side, unit.id),
    ownEscapeExits: retreatExitCount(state, { ...unit, hexId }, { unit, hexId }),
  });
}

function bestAxisOperationalPocket(state, unit, hexId, focusObjectiveHex) {
  const hypothetical = { unit, hexId };
  let best = { currentTrapExits: 6, candidateTrapExits: 6, targetValue: 0, score: 0 };
  for (const enemy of liveUnits(state.units).filter((candidate) => candidate.side === "allied" && !candidate.disrupted)) {
    const enemyDistance = distance(hexId, enemy.hexId);
    if (enemyDistance > 2) continue;
    const currentTrapExits = retreatExitCount(state, enemy, null);
    const candidateTrapExits = retreatExitCount(state, enemy, hypothetical);
    const trapGain = Math.max(0, currentTrapExits - candidateTrapExits);
    if (!trapGain && candidateTrapExits > 1) continue;

    const focusDistance = distance(enemy.hexId, focusObjectiveHex);
    const targetValue = strategicUnitValue(state, "axis", enemy)
...
function alliedForwardWallPlaybookMove(state, unit, hexId) {
  ...
  for (const axisUnit of axisAssaultUnits) {
    for (const objectiveHexId of axisTargetsForUnit(axisUnit)) {
      ...
      const lineLinks = liveUnits(state.units).filter((ally) => (
        ...
        && !ally.disrupted
        && distance(hexId, ally.hexId) === 2
        && distance(ally.hexId, objectiveHexId) >= 3
        && distance(ally.hexId, objectiveHexId) <= 9
      )).length;
      const zocCutsLane = zocHexes.some((zocHexId) => {
        const zocToAxis = distance(zocHexId, axisUnit.hexId);
        const zocToObjective = distance(zocHexId, objectiveHexId);
        return zocToAxis >= 1
          && zocToAxis <= 6
          && zocToObjective < axisToObjective
          && zocToAxis + zocToObjective <= axisToObjective + 2;
      });
      best = Math.max(best, alliedForwardWallPlaybookScore({
        turn: state.turn,
        hexToObjective,
        axisToObjective,
        axisToHex,
...
function alliedOperationalPlanMove(state, unit, hexId) {
  if (unit.side !== "allied") return 0;
  const plan = currentOperationalPlan(state, "allied");
  const axisSpearheads = Array.from(plan?.axisSpearheadIds || [])
    .map((id) => unitById(state.units, id))
  ...
  for (const axisUnit of axisSpearheads) {
    for (const objectiveHexId of axisTargetsForUnit(axisUnit)) {
      ...
      const alliedMates = liveUnits(state.units).filter((ally) => (
        ally.side === "allied"
        && ally.id !== unit.id
        && !ally.disrupted
        && nearestDistance(ally.hexId, axisObjectives()) <= 10
      ));
      const lineLinks = alliedMates.filter((ally) => distance(hexId, ally.hexId) === 2).length;
      ...
      const currentTrapExits = retreatExitCount(state, axisUnit, null);
      const candidateTrapExits = retreatExitCount(state, axisUnit, { unit, hexId });
      best = Math.max(best, alliedOperationalPlanMoveScore({
        turn: state.turn,
        role,
        hexToObjective,
        axisToObjective,
...
function axisLocalAttackOvermass(state, unit, hexId) {
  ...
  for (const enemy of liveUnits(state.units).filter((candidate) => candidate.side === "allied" && !candidate.disrupted)) {
    ...
    const otherAttackers = neighborsOf(board, enemy.hexId)
      .map((neighborId) => liveUnitAt(state.units, neighborId))
      .filter((ally) => ally && ally.side === "axis" && ally.id !== unit.id && !ally.disrupted);
    if (!otherAttackers.length) continue;
    penalty += localAttackOvermassPenalty({
      attackerSide: "axis",
      candidateCombat: Number(unit.combat || 0),
      candidateMovement: Number(unit.movement || 0),
      defense: defenseBreakdown(context(state), enemy).total,
      ...
      trappedExitCount: retreatExitCount(state, enemy, hypothetical),
      targetObjective: axisObjectives().includes(enemy.hexId),
    });
  }
  return Math.min(1800, penalty);
}

function axisSurplusBreakthrough(state, unit, hexId, route = null) {
  if (unit.side !== "axis") return 0;
  const combat = Number(unit.combat || 0);
  const movement = Number(unit.movement || 0);
  if (combat < 3 && movement < 6) return 0;

  const currentOvermass = axisLocalAttackOvermass(state, unit, unit.hexId);
  const candidateOvermass = axisLocalAttackOvermass(state, unit, hexId);
  ...
  const mates = liveUnits(state.units).filter((ally) => ally.side === "axis" && ally.id !== unit.id && !ally.disrupted);
  const exactLinks = mates.filter((ally) => distance(hexId, ally.hexId) === 2).length;
  const adjacentCrowd = mates.filter((ally) => distance(hexId, ally.hexId) === 1).length;
  ...
  return clampScore(score * mobileFit * (efficientMove ? 1 : 0.58), -120, 520);
}
...
function axisForwardScreenTempo(state, unit, hexId) {
  if (unit.side !== "axis" || isAxisAssaultUnit(unit)) return 0;
  const units = liveUnits(state.units);
  const combat = Number(unit.combat || 0);
  const currentObjectiveDistance = nearestDistance(unit.hexId, axisObjectives());
  ...
  const assaultUnits = units.filter((candidate) => isAxisAssaultUnit(candidate) && !candidate.disrupted);
  const nearestAssault = assaultUnits.length
    ? Math.min(...assaultUnits.map((assault) => distance(hexId, assault.hexId)))
    : Infinity;
  const progress = currentObjectiveDistance - candidateObjectiveDistance;
  const axisMates = units.filter((ally) => ally.side === "axis" && ally.id !== unit.id && !ally.disrupted);
  const exactLinks = axisMates.filter((ally) => distance(hexId, ally.hexId) === 2).length;
  const looseLinks = axisMates.filter((ally) => distance(hexId, ally.hexId) === 3).length;
  const adjacentCrowd = axisMates.filter((ally) => distance(hexId, ally.hexId) === 1).length;
  const closeCrowd = axisMates.filter((ally) => distance(hexId, ally.hexId) <= 2).length;
  const westExit = scenario.objectives.alliedWestExitEdge;
  const uncoveredExitThreat = westExit.reduce((sum, exitHexId) => (
  ...
  if (candidateObjectiveDistance <= 3 && state.turn <= 2) score -= (4 - candidateObjectiveDistance) * (combat >= 3 ? 18 : 10);
  ...
  return clampScore(score, -220, 320);
}
...
function alliedFinalGateScreen(state, unit, hexId) {
  if (unit.side !== "allied" || state.turn < 2) return 0;
  const axisAssaultUnits = liveUnits(state.units).filter((candidate) => isAxisAssaultUnit(candidate) && !candidate.disrupted);
  if (!axisAssaultUnits.length) return 0;
  ...
  for (const objectiveHexId of axisObjectives()) {
    ...
    const zocCutsLane = zocHexes.some((zocHexId) => {
      const zocToAxis = distance(zocHexId, axisThreat.axisUnit.hexId);
      const zocToObjective = distance(zocHexId, objectiveHexId);
      return zocToAxis >= 1
        && zocToAxis <= 5
    ...
    score += finalGateScreenScore({
      turn: state.turn,
      hexToObjective,
      axisToObjective: axisThreat.axisToObjective,
      axisToHex: axisThreat.axisToHex,
      gateCount: roadGateCount(state, objectiveHexId, unit.id),
      lineLinks,
      occupiedByAllied: liveUnitAt(state.units, objectiveHexId)?.side === "allied",
      zocCutsLane,
      combat: Number(unit.combat || 0),
...
function alliedSpearheadCounterattackPosition(state, unit, hexId) {
  ...
  for (const axisUnit of liveUnits(state.units).filter((candidate) => isAxisAssaultUnit(candidate) && !candidate.disrupted)) {
    ...
    const nearbyAxisSupport = liveUnits(state.units)
      .filter((candidate) => candidate.side === "axis" && candidate.id !== axisUnit.id && !candidate.disrupted && distance(candidate.hexId, axisUnit.hexId) <= 2)
